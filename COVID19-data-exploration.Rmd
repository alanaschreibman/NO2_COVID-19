...
###
find mean of median age, and set half above mean and half below mean as factors
make bar plot and fill with the above/below mean (but dodge) and put covid counts on y axis
```{r, eval = TRUE}
...
cases.med.age <- function(x) { 
                    ...
                    mutate(med.age = factor(med.age, levels = c(1, 2), labels = c("below.med", "above.med"))) %>%
...
owid.countries %>%
        ...
        ggplot(aes(x = ))
owid.med.age
max(owid.countries$med.age, na.rm = TRUE)
cases.med.age
```
